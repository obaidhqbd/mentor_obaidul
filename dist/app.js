...
async function loadProject(c) {
  ...
  for (const rel of c.files) {
    ...
    try { const res = await fetch(url); if (!res.ok || isBinary(rel)) { files.set(rel, {binary:true,url,original:null}); continue; } files.set(rel, {binary:false,url,original:await res.text()}); }
    catch { files.set(rel, {binary:true,url,original:null}); }
  }
  ...
  editable.forEach(rel => { const saved = stored?.files?.[rel]; if (saved != null && files.get(rel)?.binary === false) files.get(rel).current = saved; else files.get(rel).current = files.get(rel).original; });
  renderFileList(editable);
  selectFile(activeClass.entry && files.has(activeClass.entry) ? activeClass.entry : editable.find(r => /\.html?$/i.test(r)) || editable[0]);
  updatePreview();
}

function renderFileList(editable) {
  $('#fileList').innerHTML = editable.map(rel => `<button class="file-btn" data-file="${escapeAttr(rel)}"><span>${fileIcon(rel)}</span><span>${escapeHtml(rel)}</span></button>`).join('');
  $('#fileList').querySelectorAll('.file-btn').forEach(btn => btn.onclick = () => selectFile(btn.dataset.file));
}
...
function selectFile(rel) {
  ...
  const extensions = [lineNumbers(), highlightActiveLine(), highlightActiveLineGutter(), drawSelection(), history(), keymap.of([...defaultKeymap,...historyKeymap,indentWithTab, ...completionKeymap, ...closeBracketsKeymap]), bracketMatching(), closeBrackets(), foldGutter(), indentOnInput(), syntaxHighlighting(defaultHighlightStyle, {fallback:true}), autocompletion({activateOnTyping:true}), language, EditorView.updateListener.of(v => { if (v.docChanged) onEdit(); })];
  editorView = new EditorView({state:EditorState.create({doc:item.current||'',extensions}),parent:$('#codeHost')});
  $('#languageStatus').textContent = extensionLabel(rel); $('#saveStatus').textContent = localExists(activeClass.id) ? 'Local changes available' : 'Original project';
}
...
function injectProject(htmlText, entryFile) {
  const base = `https://example.invalid/${activeClass.slug}/`;
  ...
  const assetBase = `classes/${encodeURIComponent(activeClass.slug)}/`;
